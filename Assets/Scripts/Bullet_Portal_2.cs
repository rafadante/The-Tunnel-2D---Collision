...
public class Bullet_Portal_2 : MonoBehaviour {
	...
	public GameObject portal_2;//prefab com a animação da explosão
	...
	void OnTriggerEnter2D(Collider2D other)
	{
		if(other.gameObject.tag == "side_box")
		{
			if(!verifica)
			{
				Instantiate(portal_2, this.transform.position, this.transform.rotation);//portal é instaciada no local onde ocorreu a colisão
